		...
		static void LoadBulletList ()
		{
			if ((bulletList.Count <= 0) && (bulletMaterial))
			{
				for (int i = 0; i < 20; i++)
				{
					...
					{
						bulletList.Add (DecalInGameManager.DECAL_INGAME_MANAGER.CreateNewMeshDecal (bulletMaterial, null, BasicDefines.TOO_FAR_POSITION, Vector3.one, 4, Vector2.zero, false,-1));
					}
				}
			}
		} 
